...
class PositionListController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $totalData = EmployeePosition::where(function($query) {
                        })
                        ->count();
        ...
        if(empty($search))
        {
            $employeePositions = EmployeePosition::leftJoin('departments as dep', 'employee_positions.position_for_department_id', '=', 'dep.id')
                        ->select('employee_positions.*', 'dep.name as department_name') // Iekļauj parent.name
                        ->offset($start)
                        ->limit($limit)
                        ->orderBy($order, $dir)
                        ->get();
            ...
            $totalFiltered = EmployeePosition::leftJoin('departments as dep', 'employee_positions.position_for_department_id', '=', 'dep.id')
                                    ->select('employee_positions.*', 'dep.name as department_name') // Iekļauj parent.name
                                    ->where(function($query) use ($search) {
                                        self::setWhereFor($query, $search);
                                    })
        ...
        if(!empty($employeePositions))
        {
            ...
            {
                $nestedData['guid'] = $employeePosition->id;
                $nestedData['position_name'] = $employeePosition->position_name;
                $nestedData['department_name'] = $employeePosition->department_name;
                $nestedData['is_head'] = $employeePosition->is_head;
                $data[] = $nestedData;
            }
        }
    ...
    public static function setWhereFor(&$query, $search){
        $query//->where('employee_positions.position_name', 'LIKE', "'%{$search}%'")
    ...
    public static function search_position(Request $request)
    {
        return EmployeePosition::where('position_name', 'LIKE', '%' . $request->query('query') . '%')
                                        ->orderBy('created_at', 'asc') // Order by id in ascending order
                                        ->limit(10) // Limit to 10 record
                                        ->get(['id', 'position_name', 'position_for_department_id']);
    }
