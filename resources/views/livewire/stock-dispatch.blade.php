<div class="py-5" style="padding: 0px !important;">
            <table id="stock-dispatch-table" class="display table table-striped table-bordered" style="width:100%">
                <thead>
                    <tr>
                        <th>Id</th> <!-- `guid` -->
                        <th>Tips</th> <!-- `type` -->
                        <th>Izsniegts</th> <!-- `quantity` -->
    ...
    <livewire:stock-dispatch-form />  
</div>
<script>
    document.addEventListener('livewire:initialized', function () {
        // Inicializē DataTable
        var $table2 = $('#stock-dispatch-table').DataTable({
            // deferLoading: 0,
            processing: true,
            serverSide: true,
            stateSave: true,
        ...
        document.addEventListener('dataTableAjaxReload-stock-dispatch', function () {
            $table2.ajax.reload(null, false); // Nepārstartē lapošanu pēc atjaunināšanas
        });

        var $tableBody2 = $table2.table().body();
        // Pievieno klikšķa notikumu katrai rindai
        $($tableBody2).on('click', 'td.first-element', function () {
            var Id = $(this).closest('tr').data('id');
            if (!Id) {
                console.log('ID not found!');
                // toastr.warning("ID not found!");
            }
            Livewire.dispatch('openModal3', {id: Id});
        });
    });
</script>
