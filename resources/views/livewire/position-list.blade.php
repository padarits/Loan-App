<div class="py-5" style="padding: 0px !important;">
            <table id="position-list" class="display table table-striped table-bordered" style="width:100%">
                <thead>
                    <tr>
                        <th>Id</th> <!-- `email` -->
                        <th>Nosaukums</th> <!-- `email` -->
                        <th>Departaments</th> <!-- `name` -->
                        <th>Vadītājs</th> <!-- `description` -->
                    </tr>
                </thead>
            </table>
        <livewire:position-list-form />  
</div>
<script>
    document.addEventListener('livewire:initialized', function () {
        // Inicializē DataTable
        var $table = $('#position-list').DataTable({
            ...
            buttons: [
                {
                    ...
                    action: function (e, dt, node, config) {
                        Livewire.dispatch('openModalNewDocPosition');
                    }
                }
            ],
            ajax: {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        return '<input disabled ' + checked + ' type="checkbox" class="" data-row-id="${meta.row}" />';
                    },
                    orderable: false, // Lai checkbox kolonna nebūtu kārtojama
                    searchable: false // Ja checkbox nav nepieciešams meklēšanā
                }
            ],
            columnDefs: [
            {
        ...
        document.addEventListener('dataTableAjaxReload-positions', function () {
        ...
        $($tableBody).on('click', 'td.first-element', function () {
            ...
            const doc = document.getElementById("position-list-form");
            Livewire.find(doc.getAttribute("wire:id")).dispatch('openModalPosition', {id: Id});
        });

        $('#position-list tbody').on('input', 'td[contenteditable="true"]', function() {
